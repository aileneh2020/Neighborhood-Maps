...
class DisplayMap extends Component {
  ...
  getVenueData = (props, fsData) => {
    ...
    return fsData.response.venues.filter(
  ...
  getListVenueData = (listMarker, fsData) => {
    ...
    return fsData.response.venues.filter(
      item =>
        item.name.includes(listMarker.name) ||
        listMarker.name.includes(item.name)
    );
  };
  ...
  onListClick = (props, listMarker) => {
    ...
    fetch(request)
      ...
      .then(result => {
        ...
        if (this.state.currMarker.fs) {
          let detailUrl = `https://api.foursquare.com/v2/venues/${
            currRestaurant[0].id
          }/photos?client_id=${fsClientID}&client_secret=${fsClientSecret}&v=${fsVersion}&limit=1`;
          fetch(detailUrl)
            .then(response => response.json())
            .then(result => {
              console.log(result);
    ...
    if (this.props.showInfoWindow) {
      this.state.activeMarker.setAnimation(
        this.props.google.maps.Animation.null
      );
    }
                ...
                {this.state.currMarker.image ? (
                  <div>
                    <img
                      ...
                      src={
                        this.state.currMarker.image.items[0].prefix +
                        "cap100" +
                        this.state.currMarker.image.items[0].suffix
                      }
                    />
